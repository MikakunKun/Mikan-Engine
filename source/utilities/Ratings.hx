...
class Ratings
{
    ...
    public static function getRating(time:Float)
    {
        var timings:Array<Array<Dynamic>> = [
            [ClientPrefs.marvelousWindow, "marvelous"],
            [ClientPrefs.sickWindow, "sick"],
            [ClientPrefs.goodWindow, "good"],
            [ClientPrefs.badWindow, "bad"]
        ];
        ...
        for(x in timings)
        {
            if(x[1] == "marvelous" && ClientPrefs.MarvelouTrue || x[1] != "marvelous")
            {
                if(time <= x[0] * PlayState.instance.playbackRate && rating == 'bruh')
                {
                    rating = x[1];
                }
            }
    ...
    public static function getRank(accuracy:Float, ?misses:Int)
    {
        ...
        switch(ClientPrefs.LeatherratingType.toLowerCase())
        {
            case "complex":
                conditions = [
                    accuracy == 100, // SSSS
            ...
            if(rating_success)
            {
                switch(ClientPrefs.LeatherratingType.toLowerCase())
                {
                    ...
                    case "jpsych":
                        ...
                        {
                            case 0:
                                return "レーティング: " + "パーフェクト!!" + missesRating;
                            case 1:
                                return "レーティング: " + "かっこいい!" + missesRating;
                            case 2:
                                return "レーティング: " + "グッド!" + missesRating;
                            case 3:
                                return "レーティング: " + "いいね!" + missesRating;
                            case 4:
                                return "レーティング: " + "ナイス!" + missesRating;
                            case 5:
                                return "レーティング: " + "あぁ..." + missesRating;
                            case 6:
                                return "レーティング: " + "まじかよ..." + missesRating;
                            case 7:
                                return "レーティング: " + "悪い!" + missesRating;
                            case 8:
                                return "レーティング: " + "ダメ!" + missesRating;
                            case 9:
                                return "レーティング: " + "最悪!" + missesRating;
                        }
                    default:
                        switch(condition)
                        {
        ...
        if(ClientPrefs.LeatherratingType != "psych")
            return "N/A";
        else
            return "Rating: ?";

        if(ClientPrefs.LeatherratingType != "jpsych")
            return "N/A";
        else
            return "Rating: ?";
    }
