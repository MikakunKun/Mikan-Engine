...
class Note extends FlxPerspectiveSprite
{
	//////////////////////////////////////////////////
	//Extra keys stuff

	//Important stuff
	public static var gfxLetter:Array<String> = ['purple', 'blue', 'green', 'red', 'white', 'yellow', 'violet', 'darkred', 'dark',
	'pink', 'turq', 'emerald', 'lightred', 'orange', 'lime', 'darkpurple', 'darkorange', 'cobalt'];

	public static var oldgfxLetter:Array<String> = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I',
	'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R'];
	...
	public static var noteWidths:Array<Float> = EKData.Widths;
	...
	public var extraKey:Bool = false;

	public var opponentSing:Bool = false;
	...
	public var daStage:String = PlayState.curStage;
	...
	private function set_noteType(value:String):String {
		...
		{
			if (Note.ammo[PlayState.mania] <= 11)
			{
			...
			switch(value) {
				case 'Latex Note':
					...
					if(ClientPrefs.downScroll){
						reloadNote('customNotes/LATEXDOWNSCROLL');
					}else{
						reloadNote('customNotes/LATEX');
					}
					colorSwap.hue = 0;
					colorSwap.saturation = 0;
					if(daStage == 'puroStage'){
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?character:Int = 0, ?characters:Array<Int>, ?inEditor:Bool = false)
	{
		super();
	...
	function loadNoteAnims(prefix:String = '') {
		switch(prefix)
		{
			case 'DEATH': //too lazy to edit xmls
				for (i in 0...gfxLetter.length)
				{
					animation.addByPrefix(gfxLetter[i] + 'Scroll', 'death0'); // Normal notes
					if (isSustainNote)
					{
						animation.addByPrefix(gfxLetter[i] + 'hold', 'death hold piece'); // Hold
						animation.addByPrefix(gfxLetter[i] + 'holdend', 'death hold end'); // holdends
					}
				}
			case 'WARNING':
				for (i in 0...gfxLetter.length)
				{
					animation.addByPrefix(gfxLetter[i] + 'Scroll', 'warning0'); // Normal notes
					if (isSustainNote)
					{
						animation.addByPrefix(gfxLetter[i] + 'hold', 'warning hold piece'); // Hold
						animation.addByPrefix(gfxLetter[i] + 'holdend', 'warning hold end'); // holdends
					}
				}

			case 'customNotes/LIZA':
				for (i in 0...gfxLetter.length)
				{
					animation.addByPrefix(gfxLetter[i] + 'Scroll', oldgfxLetter[i] + '0'); // Normal notes
					if (isSustainNote)
					{
						animation.addByPrefix(gfxLetter[i] + 'hold', oldgfxLetter[i] + '0'); // Hold
						animation.addByPrefix(gfxLetter[i] + 'holdend', oldgfxLetter[i] + '0'); // holdends
					}
				}

			case 'customNotes/PIKA':
				for (i in 0...gfxLetter.length)
				{
					animation.addByPrefix(gfxLetter[i] + 'Scroll', oldgfxLetter[i] + '0'); // Normal notes
					if (isSustainNote)
	...
	function loadPixelNoteAnims() {
		if(isSustainNote) {
			...
			for (i in 0...gfxLetter.length) {
				animation.add(gfxLetter[i] + 'Scroll', [i + pixelNotesDivisionValue]);
			}
		}
	}
	...
	/*public function applyManiaChange()
	{
		...
		{
			animation.play(Note.keysShit.get(mania).get('letters')[noteData % Note.ammo[mania]] + 'holdend');
	...
	override function update(elapsed:Float)
	{
		...
		*/
		...
		{
			if (PlayState.characterPlayingAs != 2 && PlayState.characterPlayingAs != 3)
			{
				canBeHit = false;
