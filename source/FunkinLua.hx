...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('ratingName', '');
		set('ratingFC', '');
		set('version', MainMenuState.mikanEngineVersion.trim());
		...
		Lua_helper.add_callback(lua, "StdInt", function(variable:Dynamic) {
			var result:Dynamic = null;
			result = Std.int(variable);
			if(result == null) Lua.pushnil(lua);
			return result;
		});
		Lua_helper.add_callback(lua, "OpenSite", function(siteURL:Dynamic) {
			if(siteURL != null) CoolUtil.browserLoad(siteURL);
		});
		...
		Lua_helper.add_callback(lua, "doTweenZ", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				PlayState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {z: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						PlayState.instance.callOnLuas('onTweenCompleted', [tag]);
						PlayState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "othercharacterDance", function(character:String, id:Int) {
			switch(character.toLowerCase()) {
				case 'dad':
					if (PlayState.instance.dad.otherCharacters != null)
					{
						PlayState.instance.dad.otherCharacters[id].dance();
					}
					else
				...
				case 'gf' | 'girlfriend':
					...
					{
						if (PlayState.instance.gf.otherCharacters != null)
						{
							PlayState.instance.gf.otherCharacters[id].dance();
						}
						else
				...
				default:
					if (PlayState.instance.boyfriend.otherCharacters != null)
					{
						PlayState.instance.boyfriend.otherCharacters[id].dance();
					}
					else
		...
		Lua_helper.add_callback(lua, "othercharacterPlayAnim", function(character:String, id:Int, anim:String, ?forced:Bool = false) {
			luaTrace("characterPlayAnim is deprecated! Use playAnim instead", false, true);
			switch(character.toLowerCase()) {
				case 'dad':
					if (PlayState.instance.dad.otherCharacters != null)
					{
						if(PlayState.instance.dad.otherCharacters[id].animOffsets.exists(anim))
							PlayState.instance.dad.otherCharacters[id].playAnim(anim, forced);
					}
					else
						if(PlayState.instance.dad.animOffsets.exists(anim))
							PlayState.instance.dad.playAnim(anim, forced);
				...
				case 'gf' | 'girlfriend':
					if (PlayState.instance.gf.otherCharacters != null)
					{
						if(PlayState.instance.gf.otherCharacters[id].animOffsets.exists(anim))
							PlayState.instance.gf.otherCharacters[id].playAnim(anim, forced);
					}
					else
						if(PlayState.instance.gf != null && PlayState.instance.gf.animOffsets.exists(anim))
							PlayState.instance.gf.playAnim(anim, forced);
				...
				default:
					if (PlayState.instance.boyfriend.otherCharacters != null)
					{
						if(PlayState.instance.boyfriend.otherCharacters[id].animOffsets.exists(anim))
							PlayState.instance.boyfriend.otherCharacters[id].playAnim(anim, forced);
					}
					else
						if(PlayState.instance.boyfriend.animOffsets.exists(anim))
							PlayState.instance.boyfriend.playAnim(anim, forced);
	...
	function getBarFillDirection(?direction:String = '')
	{
		switch(direction.toLowerCase().trim())
		{
		}
		return FlxBarFillDirection.LEFT_TO_RIGHT;
	}
...
class ModchartBarSprite extends FlxBar
{
	public var wasAdded:Bool = false;
	//public var isInFront:Bool = false;

	public function new(?x:Float = 0, ?y:Float = 0, ?direction:FlxBarFillDirection, ?width:Int = 100, ?height:Int = 10, ?variable:String = "", ?min:Float = 0, ?max:Float = 100)
	{
		super(x, y, direction, width, height, this, variable, min, max);
		antialiasing = ClientPrefs.globalAntialiasing;
	}
}
...
class HScript
{
	...
	public function new()
	{
		...
		interp.variables.set('FlxSprite', FlxPerspectiveSprite);
