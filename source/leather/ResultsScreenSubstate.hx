...
class ResultsScreenSubstate extends MusicBeatSubstate
{
    public function new()
    {
        ...
        var topString = PlayState.SONG.song + " - " + CoolUtil.difficultyString() + " complete! (" + PlayState.instance.playStailText + ", " + ClientPrefs.getGameplaySetting('songspeed', 1) + "x)"; // Std.string(PlayState.songMultiplier) + "x)";
        ...
        var ratings:FlxText = new FlxText(0, FlxG.height, 0, PlayState.instance.returnStupidRatingText(true));
        ...
        bottomText.setPosition(FlxG.width - bottomText.width - 2, FlxG.height - 96); // (!PlayState.playingReplay ? 96 : 32));
