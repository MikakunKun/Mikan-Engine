...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('Mikan-Engine-logo')); // no more newground
		add(ngSpr);
		ngSpr.visible = false;
		ngSpr.setGraphicSize(Std.int(ngSpr.width * 0.5), Std.int(ngSpr.height * 0.5));
		ngSpr.updateHitbox();
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 6:
					#if PSYCH_WATERMARKS
					createCoolText(['Mikan Engine by'], -40);
					#else
					createCoolText(['In association', 'with'], -40);
					#end
				case 8:
					#if PSYCH_WATERMARKS
					addMoreText('Mikankun');
					#else
