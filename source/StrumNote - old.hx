...
class StrumNote extends FlxSprite
{
	...
	public function new(x:Float, y:Float, leData:Int, player:Int, keycount:Int) {
		...
		animationArray[0] = Note.keysShit.get(keycount).get('strumAnims')[leData];
		animationArray[1] = Note.keysShit.get(keycount).get('letters')[leData];
		animationArray[2] = Note.keysShit.get(keycount).get('letters')[leData]; // jic
	...
	public function playAnim(anim:String, ?force:Bool = false) {
		...
		if(animation.curAnim == null || animation.curAnim.name == 'static') {
			...
			if (noteData > -1 && noteData < ClientPrefs.arrowHSV.length)
			{
				if (keycount <= 11)
				{
					colorSwap.hue = ClientPrefs.arrowHSV[Std.int(Note.keysShit.get(keycount).get('pixelAnimIndex')[noteData] % keycount)][0] / 360;
					colorSwap.saturation = ClientPrefs.arrowHSV[Std.int(Note.keysShit.get(keycount).get('pixelAnimIndex')[noteData] % keycount)][1] / 100;
					colorSwap.brightness = ClientPrefs.arrowHSV[Std.int(Note.keysShit.get(keycount).get('pixelAnimIndex')[noteData] % keycount)][2] / 100;
				}
				else
				{
					colorSwap.hue = 0;
