...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Max Health & Min Health', "Value 1: maxhealth(0) or minhealth(1) \nValue 2: New Value"],
		...
		['HealthBar Opposition', "Null Is Ok"],
		['otherCharacterCameraSwitch', "Value 1: Character Value dad(opponent) or boyfriend(bf or player)\nValue 2: otherCharacterId"]
	];
	...
	**/
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Mikan Engine Chart Editor", StringTools.replace(_song.song, '-', ' ') + " (" + (_song.mania + 1) + " key" + s_termination + ")");
		#end
		...
		if (_song.mania < 0 || _song.mania == null)
			_song.mania = 3;
		...
		if (_song.HardEvent)
		{
			...
			[
				...
				['HealthBar Opposition', "Null Is Ok"],
				['Change Mania', "Value 1: The new mania value (min: " + Note.minMania + "; max: " + Note.maxMania + ")\nValue 2: Skip old strum fade tween\nPut 'true' to skip it, anything else or blank to not."],
				['Normal Wave', "toggles the cool boss shit i hardcode"],
				['Up Down Wave', "toggles the cool boss shit i hardcode"],
				['Boss Event Fish', "spawns a fish (value 1 is the amount to spawn)"],
				['Boss Event Drippin', "change tentacle direction - (value 1) and the rate (value 2)"],
				['Boss Event 2', "manually spawns a tentacle"], 
		...
		text =
		"W/S か マウスホイールで、StrumLineをスクロール出来ます
		\nAで前/Dで次のセクションへ移動
		\nLeft/Rightでスナップを変更する
		\nUp/Downでスナップを使ったStrumLineの打ち込み時間の変更
		\nLeft Bracket / Right Bracketで曲の再生速度を変更します（SHIFTで高速化）
		\nALT + Left Bracket / Right Bracketで曲の再生速度をリセットします
		\nShiftキーを押しながらだと4倍速で移動します
		\nControlキーを押しながら矢印をクリックすると選択できます
		\nZでズームイン/Xでアウトします
		\n
		\nEscでChart Editor内でチャートをテストする
		\nEnterで譜面を再生する
		\nQ/Eで長押しノーツの長さを変える/増やす
		\nSpaceで曲の停止/再開";
		...
		for (i in 0...tipTextArray.length) {
			...
			tipText.y += i * 12;
			tipText.setFormat(Paths.font("jpvcr.ttf"), 14, FlxColor.WHITE, LEFT/*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
	...
	function addSongUI():Void
	{
		...
		var check_voices = new FlxUICheckBox(10, 25, null, null, "声あり", 100);
		...
		var check_char_ids = new FlxUICheckBox(check_voices.x, check_voices.y + 14, null, null, "NoteにキャラクターIdを乗せる", 100);
		...
		var saveButton:FlxButton = new FlxButton(110, 8, "セーブ", function()
		{
			saveLevel();
		});

		var reloadSong:FlxButton = new FlxButton(saveButton.x + 90, saveButton.y, "音をリロード", function()
		{
		...
		var loadAutosaveBtn:FlxButton = new FlxButton(reloadSongJson.x, reloadSongJson.y + 30, 'オートセーブをロード', function()
		{
			PlayState.SONG = Song.parseJSONshit(FlxG.save.data.autosave);
			MusicBeatState.resetState();
		});

		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'イベントをロード', function()
		{
		...
		var clear_events:FlxButton = new FlxButton(320, 310, 'イベントを消す', function()
			{
		...
		var clear_notes:FlxButton = new FlxButton(320, clear_events.y + 30, 'ノーツを消す', function()
			{
		...
		var reloadNotesButton:FlxButton = new FlxButton(noteSplashesInputText.x + 5, noteSplashesInputText.y + 20, 'ノーツをチェンジ', function() {
		...
		tab_group_song.add(new FlxText(stepperBPM.x + 100, stepperBPM.y - 15, 0, '曲のOffset:'));
		tab_group_song.add(new FlxText(stepperSpeed.x, stepperSpeed.y - 15, 0, '曲のSpeed:'));
		...
		tab_group_song.add(new FlxText(player1DropDown.x, player1DropDown.y - 15, 0, 'P1 (Boyfriend):'));
		...
		tab_group_song.add(new FlxText(noteSplashesInputText.x, noteSplashesInputText.y - 15, 0, 'ノーツスプラッシュのノーツ:'));
	...
	function addSectionUI():Void
	{
		...
		check_mustHitSection = new FlxUICheckBox(10, 15, null, null, "P1のSection", 100);
		check_mustHitSection.name = 'check_mustHit';
		check_mustHitSection.checked = _song.notes[curSec].mustHitSection;

		check_gfSection = new FlxUICheckBox(10, check_mustHitSection.y + 22, null, null, "GFのSection", 100);
		...
		var copyButton:FlxButton = new FlxButton(10, 190, "Sectionをコピー", function()
		{
			notesCopied = [];
		...
		var pasteButton:FlxButton = new FlxButton(copyButton.x + 100, copyButton.y, "Sectionをペースト", function()
		{
			...
			for (note in notesCopied)
			{
				...
				else
				{
					...
					{
						if(note[5] != null) {
							copiedNote = [newStrumTime, note[1], note[2], note[3], note[4], note[5]];
						} else {
		...
		check_notesSec = new FlxUICheckBox(10, clearSectionButton.y + 25, null, null, "ノーツ達", 100);
		check_notesSec.checked = true;
		check_eventsSec = new FlxUICheckBox(check_notesSec.x + 100, check_notesSec.y, null, null, "イベント達", 100);
		check_eventsSec.checked = true;

		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "sectionを交換", function()
		{
			for (i in 0..._song.notes[curSec].sectionNotes.length)
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
		...
		var copyLastButton:FlxButton = new FlxButton(10, swapSection.y + 30, "最後のSectionをコピーする", function()
		{
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "ノーツをデュエット", function()
		{
		...
		tab_group_section.add(new FlxText(stepperBeats.x, stepperBeats.y - 15, 0, 'Sectionごとのビート:'));
	...
	function addNoteUI():Void
	{
		...
		var clearSectionButton:FlxButton = new FlxButton(10, 150, "クリア", clearSection);

		var clearLeftSectionButton:FlxButton = new FlxButton(clearSectionButton.x + clearSectionButton.width + 2, 150, "Leftをクリア", function()
		{
			clearSectionSide(0);
		});

		var clearRightSectionButton:FlxButton = new FlxButton(clearSectionButton.x + clearSectionButton.width + 2, 170, "Rightをクリア", function()
		{
			clearSectionSide(1);
		});

		var setCharacterLeftSide:FlxButton = new FlxButton(stepperCharLength.x, stepperCharLength.y + stepperCharLength.height + 1, "LeftにノーツIdを置く", function()
		{
			characterSectionSide(0, Std.int(stepperCharLength.value));
		});

		var setCharacterRightSide:FlxButton = new FlxButton(setCharacterLeftSide.x + setCharacterLeftSide.width + 2, setCharacterLeftSide.y, "RightにノーツIdを置く", function()
		{
			characterSectionSide(1, Std.int(stepperCharLength.value));
		});

		strumTimeInputText = new FlxUIInputText(10, 210, 180, "0");
		...
		noteTypeDropDown = new FlxUIDropDownMenuCustom(10, 250, FlxUIDropDownMenuCustom.makeStrIdLabelArray(displayNameList, true), function(character:String)
		{
		...
		characterGroup_Input = new FlxUIInputText(setCharacterLeftSide.x, setCharacterLeftSide.y + setCharacterLeftSide.height + 20, 70, "", 8);
		...
		var charGroupText = new FlxText(characterGroup_Input.x + characterGroup_Input.width + 1, characterGroup_Input.y, 0, "Characterグループ (文字列は ',' で区切って)", 9);
		
		tab_group_note.add(stepperCharLength);
		tab_group_note.add(charText);

		tab_group_note.add(setCharacterLeftSide);
		tab_group_note.add(setCharacterRightSide);

		tab_group_note.add(characterGroup_Input);
		tab_group_note.add(charGroupText);

		tab_group_note.add(clearSectionButton);
		tab_group_note.add(clearLeftSectionButton);
		tab_group_note.add(clearRightSectionButton);
		tab_group_note.add(new FlxText(10, 10, 0, '長押しノーツの長さ:'));
		tab_group_note.add(new FlxText(10, 195, 0, 'Strum time (単位：ミリ秒):'));
		tab_group_note.add(new FlxText(10, 235, 0, 'ノーツのタイプ:'));
	...
	function addEventsUI():Void
	{
		...
		var text:FlxText = new FlxText(20, 90, 0, "値 1:");
		tab_group_event.add(text);
		value1InputText = new FlxUIInputText(20, 110, 100, "");
		blockPressWhileTypingOn.push(value1InputText);

		var text:FlxText = new FlxText(20, 130, 0, "値 2:");
		tab_group_event.add(text);
		...
		selectedEventText = new FlxText(addButton.x - 100, addButton.y + addButton.height + 6, (moveRightButton.x - addButton.x) + 186, '選択したイベント: なし');
	...
	function changeEventSelected(change:Int = 0)
	{
		if(curSelectedNote != null && curSelectedNote[2] == null) //Is event note
		{
			...
			selectedEventText.text = '選択したイベント: ' + (curEventSelected + 1) + ' / ' + curSelectedNote[1].length;
		}
		else
		{
			curEventSelected = 0;
			selectedEventText.text = '選択したイベント: なし';
		}
		updateNoteUI();
	}
	...
	function addChartingUI() {
		...
		waveformUseInstrumental = new FlxUICheckBox(10, 90, null, null, "Inst(Instrumental)用波形", 100);
		...
		waveformUseVoices = new FlxUICheckBox(waveformUseInstrumental.x + 120, waveformUseInstrumental.y, null, null, "Voices用波形", 100);
		...
		check_mute_inst = new FlxUICheckBox(10, 310, null, null, "Instrumentalをミュートする(Editor内だけ)", 100);
		...
		mouseScrollingQuant = new FlxUICheckBox(10, 200, null, null, "マウス スクロール量子化", 100);
		...
		check_vortex = new FlxUICheckBox(10, 160, null, null, "ボルテックスエディター (BETA)", 100);
		...
		var check_mute_vocals = new FlxUICheckBox(check_mute_inst.x + 120, check_mute_inst.y, null, null, "Vocalsをミュートする(Editor内だけ)", 100);
		...
		playSoundBf = new FlxUICheckBox(check_mute_inst.x, check_mute_vocals.y + 30, null, null, 'Soundを流す (Boyfriendのノーツに触れた際)', 100,
			function() {
		...
		playSoundDad = new FlxUICheckBox(check_mute_inst.x + 120, playSoundBf.y, null, null, 'Soundを流す (Opponentのノーツに触れた際)', 100,
			function() {
		...
		disableAutoScrolling = new FlxUICheckBox(metronome.x + 120, metronome.y, null, null, "自動スクロールを無効にする（非推奨）", 120,
			function() {
		...
		#if !html5
		sliderRate = new FlxUISlider(this, '再生速度', 120, 120, 0.5, 3, 150, null, 5, FlxColor.WHITE, FlxColor.BLACK);
			...
			if (FlxG.keys.justPressed.TAB)
			{
				if (FlxG.keys.pressed.SHIFT)
				{
					...
					if (UI_box.selected_tab < 0)
						UI_box.selected_tab = UI_box.length - 1;
				}
				else
				{
					UI_box.selected_tab += 1;
					if (UI_box.selected_tab >= UI_box.length)
	...
	*/
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if([3] != null && note.noteType != null && note.noteType.length > 0) {
				var typeInt:Null<Int> = noteTypeMap.get(i[3]);
				var theType:String = '' + typeInt;
				if(typeInt == null) theType = '?';

				var daText:AttachedFlxText = new AttachedFlxText(0, 0, 100, theType, 24);
				daText.setFormat(Paths.font("jpvcr.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(Paths.font("jpvcr.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var goodNoteInfo = daNoteInfo % Note.ammo[_song.mania];

		var note:Note = new Note(daStrumTime, goodNoteInfo, null, null, 0, [0], true);
		...
		if(daSus != null) { //Common note
			if(doFunnyNumbers)
			{
				...
				var id:FlxText = new FlxText(Math.floor((daNoteInfo + 1) * GRID_SIZE), Math.floor(getYfromStrum((daStrumTime - sectionStartTime()))), GRID_SIZE, Std.string(i[4]).replace("[", "").replace("]", ""), 16);
	...
	function clearSectionSide(side:Int = 0):Void
	{
		...
		for(noteIndex in 0..._song.notes[curSec].sectionNotes.length)
		{
			if(side == 0)
			{
				if(_song.notes[curSec].sectionNotes[noteIndex][1] < (Note.ammo[_song.mania]))
				{
					removeThese.push(_song.notes[curSec].sectionNotes[noteIndex]);
				}
			}
			else if(side == 1)
			{
		...
		if(removeThese != [])
		{
			...
			{
				_song.notes[curSec].sectionNotes.remove(x);
			}
	...
	function characterSectionSide(side:Int = 0, character:Int = 0):Void
	{
		...
		for(noteIndex in 0..._song.notes[curSec].sectionNotes.length)
		{
			var noteData = _song.notes[curSec].sectionNotes[noteIndex][1];
	
			if(side == 0)
			{
				if(noteData < (Note.ammo[_song.mania]))
				{
		...
		if(changeThese != [])
		{
			for(x in changeThese)
			{
				...
				{
					_song.notes[curSec].sectionNotes[x][4] = character;
				}
				else
				{
					_song.notes[curSec].sectionNotes[x][4] = charactersArray;
				}
			}
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if(noteData > -1)
		{
			...
			if(characterGroup_Input.text != '' && characterGroup_Input.text != ' ')
			{
				...
				for(char in yes)
				{
					charactersDatas.push(Std.parseInt(char));
				}
			}

			_song.notes[curSec].sectionNotes.push([noteStrum, noteData, noteSus, noteTypeIntMap.get(daType), (charactersDatas.length <= 0 ? current_Note_Character : charactersDatas)]);

			curSelectedNote = _song.notes[curSec].sectionNotes[_song.notes[curSec].sectionNotes.length - 1];

			if (FlxG.keys.pressed.CONTROL)
			{
				_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + 4) % 8, noteSus, noteTypeIntMap.get(daType), (charactersDatas.length <= 0 ? current_Note_Character : charactersDatas)]);
			}
		}
		else
		{
