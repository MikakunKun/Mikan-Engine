...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		timeTxt.setFormat(Paths.font("jpvcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		changeModeText.setFormat(Paths.font("jpvcr.ttf"), 32, FlxColor.WHITE, CENTER);
	...
	override public function update(elapsed:Float)
	{
		...
		if(controls.BACK)
		{
			...
			CustomFadeTransition.nextCamera = camOther;
			MusicBeatState.switchState(new japanese.options.OptionsState());
	...
	function createTexts()
	{
		for (i in 0...4)
		{
			...
			text.setFormat(Paths.font("jpvcr.ttf"), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function updateNoteDelay()
	{
		...
		timeTxt.text = '現在のオフセット： ' + Math.floor(barPercent) + ' ms';
	}
	...
	function updateMode()
	{
		...
		if(onComboMenu)
			changeModeText.text = '< コンボオフセット（Enterキーで次項目に移動します。）>';
		else
			changeModeText.text = '< ノーツ・ビートディレイ（Enterキーで次項目に移動します。）>';
