...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['BF Change Mania', "Value 1: The new mania value (min: " + Note.minMania + "; max: " + Note.maxMania + ")\nValue 2: Skip old strum fade tween\nPut 'true' to skip it, anything else or blank to not."],
		['Dad Change Mania', "Value 1: The new mania value (min: " + Note.minMania + "; max: " + Note.maxMania + ")\nValue 2: Skip old strum fade tween\nPut 'true' to skip it, anything else or blank to not."],
	...
	**/
	...
	override function create()
	{
		...
		PlayState.playerKeyCount = _song.playerKeyCount;
		PlayState.keyCount = _song.keyCount;

		#if desktop
		var s_termination = "s";
		if ((_song.keyCount + _song.playerKeyCount) / 2 == 0) s_termination = "";
		...
		if (_song.playerKeyCount < 0 || _song.playerKeyCount == null)
			_song.playerKeyCount = Note.defaultMania + 1;

		if (_song.keyCount < 0 || _song.keyCount == null)
			_song.keyCount = Note.defaultMania + 1;
		...
		for (i in 0...(_song.keyCount + _song.playerKeyCount)){
			var note:StrumNote = new StrumNote(GRID_SIZE * (i+1), strumLine.y, i % (!_song.notes[curSec].mustHitSection ? _song.keyCount : _song.playerKeyCount), 0, (!_song.notes[curSec].mustHitSection ? _song.keyCount : _song.playerKeyCount));
	...
	function addSongUI():Void
	{
		...
		var check_char_ids = new FlxUICheckBox(check_voices.x, check_voices.y + 14, null, null, "Character Ids On Notes", 100);
		...
		var stepperKeyCount:FlxUINumericStepper = new FlxUINumericStepper(100, stepperSpeed.y, 1, 3, Note.minMania + 1, Note.maxMania + 1, 1);
		...
		var stepperPlayerKeyCount:FlxUINumericStepper = new FlxUINumericStepper(stepperKeyCount.x, stepperKeyCount.y + 15, 1, 3, Note.minMania + 1, Note.maxMania + 1, 1);
		...
		var keyCountLabel = new FlxText(stepperKeyCount.x, stepperKeyCount.y - 15, 0, "Key Count", 9);
	...
	function addSectionUI():Void
	{
		...
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Swap section", function()
		{
			...
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
				note[1] = (note[1] + (_song.playerKeyCount + _song.keyCount) / 2) % (_song.playerKeyCount + _song.keyCount);
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "Duet Notes", function()
		{
			...
			{
				var boob = note[1];
				if (boob>(_song.playerKeyCount + _song.keyCount) / 2 - 1){
					boob -= (_song.playerKeyCount + _song.keyCount) / 2;
				}else{
					boob += (_song.playerKeyCount + _song.keyCount) / 2;
				}
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Mirror Notes", function()
		{
			...
			for (note in _song.notes[curSec].sectionNotes)
			{
				var boob = note[1]%(_song.playerKeyCount + _song.keyCount) / 2;
				boob = ((_song.playerKeyCount + _song.keyCount) / 2 -1) - boob;
				if (note[1] > (_song.playerKeyCount + _song.keyCount) / 2) boob += (_song.playerKeyCount + _song.keyCount) / 2;
	...
	function addNoteUI():Void
	{
		...
		tab_group_note.add(new FlxText(10, 195, 0, 'Strum time (in miliseconds):'));
		tab_group_note.add(new FlxText(10, 235, 0, 'Note type:'));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'song_playerkeycount')
			{
	...
	override function update(elapsed:Float)
	{
		...
		var gWidth = GRID_SIZE * (_song.playerKeyCount + _song.keyCount);
		camPos.x = -80 + gWidth;
		...
		if (!blockInput)
		{
			...
			{
				autosaveSong();
				LoadingState.loadAndSwitchState(new editors.EditorPlayState(sectionStartTime(), _song.playerKeyCount, _song.keyCount));
			}
		...
		var NoteCounts:Int = (!_song.notes[curSec].mustHitSection ? _song.keyCount : _song.playerKeyCount);
		curRenderedNotes.forEachAlive(function(note:Note) {
			note.alpha = 1;
			if(curSelectedNote != null) {
				var noteDataToCheck:Int = note.noteData;
				if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += NoteCounts;
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					var data:Int = note.noteData % NoteCounts;
					var noteDataToCheck:Int = note.noteData % NoteCounts;
					if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += NoteCounts;
					...
					if(!playedSound[data]) {
						if((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)){
							...
							FlxG.sound.play(Paths.sound(soundToPlay)).pan = note.noteData < NoteCounts ? -0.3 : 0.3; //would be coolio
	...
	*/
	...
	function reloadGridLayer() {
		GRID_SIZE = Note.gridSizes[(!_song.notes[curSec].mustHitSection ? _song.keyCount + _song.playerKeyCount : _song.playerKeyCount + _song.keyCount)];
		...
		var s_termination = "s";
		if ((_song.playerKeyCount + _song.keyCount) / 2 == 0) s_termination = "";
		DiscordClient.changePresence("Mikan Engine Chart Editor", StringTools.replace(_song.song, '-', ' ') + " (" + ((_song.playerKeyCount + _song.keyCount) / 2) + " key" + s_termination + ")");
		#end

		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * (_song.keyCount + _song.playerKeyCount), Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
		...
		var oneHalf:Float = GRID_SIZE * (_song.playerKeyCount + _song.keyCount) / 2;
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * (_song.playerKeyCount + _song.keyCount), leHeight + Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
		...
		if(foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(0, gridBG.height).makeGraphic(Std.int(GRID_SIZE + GRID_SIZE * (_song.playerKeyCount + _song.keyCount)), Std.int(nextGridBG.height), FlxColor.BLACK);
			gridBlack.alpha = 0.4;
			gridLayer.add(gridBlack);
		}

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * (!_song.notes[curSec].mustHitSection ? _song.keyCount : _song.playerKeyCount))).makeGraphic(2, leHeight, FlxColor.BLACK);
		...
		strumLine = new FlxSprite(0, 50).makeGraphic(Std.int(GRID_SIZE + GRID_SIZE * (_song.playerKeyCount + _song.keyCount)), 4);
		add(strumLine);
	...
	function updateWaveform() {
		#if desktop
		if(waveformPrinted) {
			waveformSprite.makeGraphic(Std.int(GRID_SIZE * (_song.playerKeyCount + _song.keyCount)), Std.int(gridBG.height), 0x00FFFFFF);
		...
		// Draws
		var gSize:Int = Std.int(GRID_SIZE * (_song.playerKeyCount + _song.keyCount));
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[1] > (_song.playerKeyCount + _song.keyCount) / 2 - 1) note.mustPress = !note.mustPress;
		}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		if(!_song.notes[curSec].mustHitSection && mustPress)
		...
		if(isNextSection && _song.notes[curSec].mustHitSection != _song.notes[curSec+1].mustHitSection) {
			if(daNoteInfo > ((_song.keyCount + _song.playerKeyCount) / 2) - 1) {
				note.x -= GRID_SIZE * (_song.keyCount + _song.playerKeyCount) / 2;
			} else if(daSus != null) {
				note.x += GRID_SIZE * (_song.keyCount + _song.playerKeyCount) / 2;
			}
		}
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += (!_song.notes[curSec].mustHitSection ? _song.keyCount : _song.playerKeyCount);
			for (i in _song.notes[curSec].sectionNotes)
			{
	...
	{
		var noteDataToCheck:Int = note.noteData;
		if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += (!_song.notes[curSec].mustHitSection ? _song.keyCount : _song.playerKeyCount);
	...
	function clearSectionSide(side:Int = 0):Void
	{
		...
		for(noteIndex in 0..._song.notes[curSec].sectionNotes.length)
		{
			if(side == 0)
			{
				if(_song.notes[curSec].sectionNotes[noteIndex][1] < _song.keyCount)
				{
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if(noteData > -1)
		{
			...
			if (FlxG.keys.pressed.CONTROL)
			{
				_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + _song.keyCount) % (_song.keyCount + _song.playerKeyCount), noteSus, noteTypeIntMap.get(daType), (charactersDatas.length <= 0 ? current_Note_Character : charactersDatas)]);
			}
		}
		else
		{
