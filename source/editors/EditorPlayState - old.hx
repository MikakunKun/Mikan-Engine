...
class EditorPlayState extends MusicBeatState
{
	...
	var PlayerNote:Int = 0;
	var DadNote:Int = 0;

	public function new(startPos:Float, PlayerNote:Int = 0, DadNote:Int = 0) {
	...
	override function create()
	{
		...
		PlayState.playerKeyCount = PlayerNote;
			...
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					if(daStrumTime >= startPos) {
						...
						if(songNotes[1] >= (!gottaHitNote ? DadNote : PlayerNote))
						{
						...
						var daNoteData:Int = Std.int(songNotes[1] % (!gottaHitNote ? DadNote : PlayerNote));
				...
				if(daNote.copyY) {
					if (ClientPrefs.downScroll) {
						...
						if (daNote.isSustainNote) {
							...
							if (characterPlayingAs == 0)
								if(!daNote.mustPress)
									daNote.y += 27.5 * ((PlayState.SONG.bpm / 100) - 1) * (PlayState.SONG.speed - 1) * Note.scales[DadNote];
								else
									daNote.y += 27.5 * ((PlayState.SONG.bpm / 100) - 1) * (PlayState.SONG.speed - 1) * Note.scales[PlayerNote];
							else
								if(!daNote.mustPress)
									daNote.y += 27.5 * ((PlayState.SONG.bpm / 100) - 1) * (PlayState.SONG.speed - 1) * Note.scales[PlayerNote];
							else
	...
	private function getKeyFromEvent(key:FlxKey):Int
	{
		if(key != NONE)
		{
			for (i in 0...keysArray[PlayerNote].length)
			{
				for (j in 0...keysArray[PlayerNote][i].length)
				{
					if(key == keysArray[PlayerNote][i][j])
					{
						return i;
					}
				}
	...
	private function keysArePressed():Bool
	{
		for (i in 0...keysArray[PlayerNote].length) {
			for (j in 0...keysArray[PlayerNote][i].length) {
				if (FlxG.keys.checkStatus(keysArray[PlayerNote][i][j], PRESSED)) return true;
			}
		}
	...
	{
		for (i in 0...keysArray[PlayerNote][data].length) {
			if (FlxG.keys.checkStatus(keysArray[PlayerNote][data][i], PRESSED)) return true;
		}
	...
	{
		var usedKeyCount = DadNote;

		if(isPlayer)
			usedKeyCount = PlayerNote;
		...
		for (i in 0...usedKeyCount)
		{
			...
			var babyArrow:StrumNote = new StrumNote(ClientPrefs.middleScroll ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X, strumLine.y, i, player, usedKeyCount);
			...
			else
			{
				...
				{
					babyArrow.x += 310;
					if(i > Note.separator[usedKeyCount]) { //Up and Right
	...
	function spawnNoteSplashDadOnNote(note:Note) {
		if(ClientPrefs.OpponentnoteSplashes && ClientPrefs.opponentStrums && note != null) {
			...
			if(strum != null) {
				DadspawnNoteSplash(strum.x, strum.y, note.noteData, note);
			}
		}
	}
	...
	function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		var hue:Float = ClientPrefs.arrowHSV[Std.int(Note.keysShit.get(PlayerNote).get('pixelAnimIndex')[data] % PlayerNote)][0] / 360;
		var sat:Float = ClientPrefs.arrowHSV[Std.int(Note.keysShit.get(PlayerNote).get('pixelAnimIndex')[data] % PlayerNote)][1] / 100;
		var brt:Float = ClientPrefs.arrowHSV[Std.int(Note.keysShit.get(PlayerNote).get('pixelAnimIndex')[data] % PlayerNote)][2] / 100;
		if(note != null) {
	...
	function DadspawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		var skin:String = 'noteSplashes';
		if(PlayState.SONG.splashSkin != null && PlayState.SONG.splashSkin.length > 0) skin = PlayState.SONG.splashSkin;
		
	
		var hue:Float = ClientPrefs.arrowHSV[Std.int(Note.keysShit.get(DadNote).get('pixelAnimIndex')[data] % DadNote)][0] / 360;
		var sat:Float = ClientPrefs.arrowHSV[Std.int(Note.keysShit.get(DadNote).get('pixelAnimIndex')[data] % DadNote)][1] / 100;
		var brt:Float = ClientPrefs.arrowHSV[Std.int(Note.keysShit.get(DadNote).get('pixelAnimIndex')[data] % DadNote)][2] / 100;
		if(note != null) {
			skin = note.noteSplashTexture;
			if (DadNote >= 12)
			{
