...
class CharacterEditorState extends MusicBeatState
{
	...
	function addSettingsUI() {
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			for (character in characters)
			{
				...
				if (parsedJson.rating_position == null)
					character.ratingPosition = [0, 0];
	...
	var positionRatingXStepper:FlxUINumericStepper;
	var positionRatingYStepper:FlxUINumericStepper;
	...
	function addCharacterUI() {
		...
		if (char.ratingPosition == null)
		{
			positionRatingXStepper = new FlxUINumericStepper(positionRatingXStepper.x, positionRatingXStepper.y + 80, 10, 0, -9000, 9000, 0);
			positionRatingYStepper = new FlxUINumericStepper(positionRatingYStepper.x, positionRatingYStepper.y + 80, 10, 0, -9000, 9000, 0);
		}

		if (char.ratingPosition != null)
		{
			positionRatingXStepper = new FlxUINumericStepper(positionRatingXStepper.x, positionRatingXStepper.y + 80, 10, char.ratingPosition[0], -9000, 9000, 0);
			positionRatingYStepper = new FlxUINumericStepper(positionRatingYStepper.x, positionRatingYStepper.y + 80, 10, char.ratingPosition[1], -9000, 9000, 0);
		}
		...
		tab_group.add(new FlxText(positionRatingXStepper.x, positionRatingXStepper.y - 18, 0, 'Rating X/Y:'));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == positionRatingXStepper)
			{
				char.ratingPosition[0] = positionRatingXStepper.value;
				updatePointerPos();
			}
			else if(sender == positionRatingYStepper)
			{
				char.ratingPosition[1] = positionRatingYStepper.value;
				updatePointerPos();
			}
	...
	function updatePointerPos() {
		...
		if(!char.isPlayer) {
			...
			x += 150 + char.ratingPosition[0];
		} else {
			x -= 100 + char.cameraPosition[0];
			x -= 100 + char.ratingPosition[0];
		}
		y -= 100 - char.cameraPosition[1];
		y -= 100 - char.ratingPosition[1];
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			positionCameraXStepper.value = char.ratingPosition[0];
			positionCameraYStepper.value = char.ratingPosition[1];
			reloadAnimationDropDown();
			updatePresence();
		}
	}
	...
	function saveCharacter() {
		var json = {
			...
			"rating_position": char.ratingPosition,
