...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var dadvocals:FlxSound = null;
	var bfvocals:FlxSound = null;
	...
	override function create()
	{
		...
		_song.specialAudioName = CoolUtil.difficultyString();
		// Paths.clearMemory();
	...
	function addChartingUI() {
		...
		var vocalsExtraTwo = new FlxUICheckBox(check_mute_inst.x, check_mute_vocals.y + 40, null, null, "Has Dad voice track", 100);
		vocalsExtraTwo.checked = _song.needsBfVoices;
		// _song.needsVoices = check_voices.checked;
		vocalsExtraTwo.callback = function()
		{
			_song.needsBfVoices = vocalsExtraTwo.checked;
			//trace('CHECKED!');
		};

		var vocalsExtraOne = new FlxUICheckBox(check_mute_inst.x + 120, vocalsExtraTwo.y, null, null, "Has Bf voice track", 100);
		vocalsExtraOne.checked = _song.needsDadVoices;
		// _song.needsVoices = check_voices.checked;
		vocalsExtraOne.callback = function()
		{
			_song.needsDadVoices = vocalsExtraOne.checked;
			//trace('CHECKED!');
		};

		playSoundBf = new FlxUICheckBox(check_mute_inst.x, check_mute_vocals.y + 20, null, null, 'Play Sound (Boyfriend notes)', 100,
			function() {
	...
	function loadSong():Void
	{
		...
		var filedad:Dynamic = Paths.dadvoices(currentSongName, _song.player1);
		var filebf:Dynamic = Paths.bfvoices(currentSongName, _song.player2);
		vocals = new FlxSound();
		...
		if (Std.isOfType(filedad, Sound) || OpenFlAssets.exists(filedad)) {
			if (_song.needsDadVoices)
				dadvocals.loadEmbedded(filedad);
			FlxG.sound.list.add(dadvocals);
		}
		bfvocals = new FlxSound();
		if (Std.isOfType(filebf, Sound) || OpenFlAssets.exists(filebf)) {
			if (_song.needsBfVoices)
				bfvocals.loadEmbedded(filebf);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'voices_volume')
			{
				...
				dadvocals.volume = nums.value;
				bfvocals.volume = nums.value;
			}
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				if(dadvocals != null) dadvocals.stop();
				if(bfvocals != null) bfvocals.stop();
			...
			backkey = ClientPrefs.copyKey(ClientPrefs.keyBinds.get('backkey'));

			if (FlxG.keys.anyJustPressed(backkey)) {
			...
			if (FlxG.keys.justPressed.TAB)
			{
				...
				{
					UI_box.selected_tab += 1;
					if (UI_box.selected_tab >= UI_box.length - 1)
			...
			if (FlxG.keys.justPressed.SPACE)
			{
				if (FlxG.sound.music.playing)
				{
					...
					if(dadvocals != null) dadvocals.pause();
					if(bfvocals != null) bfvocals.pause();
				}
				else
				{
					if(vocals != null) {
		...
		dadvocals.pitch = playbackSpeed;
		bfvocals.pitch = playbackSpeed;
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		if(daSus != null) { //Common note
			if(doFunnyNumbers)
			{
				...
				if (Note.ammo[_song.mania] >= 12)
					id = new FlxText(Math.floor((daNoteInfo + 1) * GRID_SIZE), Math.floor(getYfromStrum((daStrumTime - sectionStartTime()))), GRID_SIZE, Std.string(i[4]).replace("[", "").replace("]", ""), 8);
